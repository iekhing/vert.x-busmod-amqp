...
abstract class MessageTransformingConsumer extends DefaultConsumer {
	...
	public void handleDelivery(final String consumerTag,
		...
		/*
		 * { exchange: String, routingKey: String, properties: { … }, body: { …
		 * } }
		 */
		JsonObject msg = new JsonObject().putString("exchange",
				envelope.getExchange()).putString("routingKey",
		...
		if (properties != null) {
			...
			// maybeSetProperty(jsonProps, "className",
			// properties.getClassName());

			maybeSetProperty(jsonProps, "clusterId", properties.getClusterId());
			maybeSetProperty(jsonProps, "contentEncoding",
					properties.getContentEncoding());
			maybeSetProperty(jsonProps, "correlationId",
					properties.getCorrelationId());
			maybeSetProperty(jsonProps, "deliveryMode",
					properties.getDeliveryMode());
			maybeSetProperty(jsonProps, "expiration",
			...
			if (properties.getHeaders() != null) {
				...
				for (Map.Entry<String, Object> entry : properties.getHeaders()
						.entrySet()) {
					maybeSetProperty(headersObj, entry.getKey(),
							entry.getValue());
				}
			}
		...
		try {
			...
			logger.warning("unknown content type "
					+ properties.getContentType() + "; defaulting to "
		...
		try {
			if (ContentType.JSON_CONTENT_TYPES.contains(contentType)) {
				...
				Map<String, Object> data = om.readValue(
				...
				msg.putString(
						"body",
						new String(body, jsonProps.getString("contentEncoding",
								"UTF-8")));

				decoded = true;
			}
		} catch (DecodeException e) {
			logger.log(Level.WARNING, "Unable to decode message body as "
					+ contentType, e);
	...
	// {{{ maybeSetProperty
	private void maybeSetProperty(final JsonObject json, final String key,
			final Object value) {
		if (value != null) {
			if (value instanceof String) {
				...
				try {
					...
					throw new IllegalStateException(
							"UTF-8 is not supported, eh?  Really?", e);
				}
